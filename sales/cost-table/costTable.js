...
let isUpdatingSellingPrice = false; // prevent double-submit while a request is in flight
...
// ───────────────────────────────────────────────────────────────────────────────
// Temporary inline selling price editing (Sales cost table only)
// ───────────────────────────────────────────────────────────────────────────────

let sellingPriceInputHandlerAttached = false;
let sellingPriceKeydownHandler = null;
let sellingPriceBlurHandler = null;
let sellingPriceFocusHandler = null;

function attachSellingPriceInputHandler() {
    if (!costTable || !costTable.container) return;
    
    const container = costTable.container;
    
    // Remove existing listeners if they exist (to avoid duplicates)
    if (sellingPriceInputHandlerAttached) {
        if (sellingPriceKeydownHandler) {
            container.removeEventListener('keydown', sellingPriceKeydownHandler);
        }
        if (sellingPriceBlurHandler) {
            container.removeEventListener('blur', sellingPriceBlurHandler, true);
        }
        if (sellingPriceFocusHandler) {
            container.removeEventListener('focus', sellingPriceFocusHandler, true);
        }
    }

    // Create new handlers
    sellingPriceKeydownHandler = async (event) => {
        const target = event.target;
        if (!(target instanceof HTMLInputElement)) return;
        if (target.getAttribute('data-selling-price-input') !== 'true') return;

        // Enter key commits
        if (event.key === 'Enter') {
            event.preventDefault();
            await handleSellingPriceCommit(target);
        }
    };

    sellingPriceBlurHandler = async (event) => {
        const target = event.target;
        if (!(target instanceof HTMLInputElement)) return;
        if (target.getAttribute('data-selling-price-input') !== 'true') return;
    ...
    sellingPriceFocusHandler = (event) => {
        const target = event.target;
        if (!(target instanceof HTMLInputElement)) return;
        if (target.getAttribute('data-selling-price-input') !== 'true') return;

        // Store original value when input is focused (update if not set or if value changed)
        const currentValue = target.value || '';
        const storedOriginal = target.getAttribute('data-original-value') || '';
        if (!storedOriginal || storedOriginal !== currentValue) {
            target.setAttribute('data-original-value', currentValue);
        }
    };

    container.addEventListener('keydown', sellingPriceKeydownHandler);
    container.addEventListener('blur', sellingPriceBlurHandler, true);
    container.addEventListener('focus', sellingPriceFocusHandler, true);

    sellingPriceInputHandlerAttached = true;
}

async function handleSellingPriceCommit(inputEl) {
    if (isUpdatingSellingPrice) return;
    const jobNo = inputEl.getAttribute('data-job-no');
    if (!jobNo) return;

    const rawValue = (inputEl.value || '').toString().trim();
    if (!rawValue) {
        // Empty input: do not change anything (temporary behaviour)
        return;
    }
    ...
    if (originalValue) {
        ...
        if (Number.isFinite(originalNumeric) && Math.abs(numeric - originalNumeric) < 0.01) {
            // Value hasn't changed, don't send request
            return;
        }
    }
    ...
    try {
        ...
        await patchJobCostSummary(jobNo, payload);

        // Update local data structures without reloading
        const newPrice = numeric.toFixed(2);
        
        // Update in costTableRoots
        function updateRowInArray(arr, jobNo, newPrice) {
        ...
        updateRowInArray(costTableRoots, jobNo, newPrice);
        
        // Update in childrenCache
        for (const children of childrenCache.values()) {
            updateRowInArray(children, jobNo, newPrice);
        }
        
        // Update the input value to show the formatted value
        const formattedValue = numeric.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        inputEl.value = formattedValue;
        
        // Update the original value for future comparisons
        inputEl.setAttribute('data-original-value', formattedValue);
        
        // Refresh table display
        updateTableDataOnly();
    } catch (err) {
        console.error('Selling price update failed', err);
        alert('Satış fiyatı güncellenemedi.');
        // Restore original value on error
        const originalRow = [...costTableRoots, ...Array.from(childrenCache.values()).flat()].find(r => r.job_no === jobNo);
        if (originalRow && originalRow.selling_price) {
            const originalNum = typeof originalRow.selling_price === 'string' ? parseFloat(originalRow.selling_price) : originalRow.selling_price;
            inputEl.value = originalNum ? originalNum.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '';
        }
    } finally {
    ...
    costTable = new TableComponent('cost-table-container', {
        ...
        columns: [
            ...
            { field: 'actual_total_cost', label: 'Toplam Maliyet', sortable: true, formatter: v => `<span class="fw-bold">${formatMoney(v)}</span>` },
            {
                ...
                sortable: true,
                formatter: (v, row) => {
                    const current = v != null && v !== '' ? Number(v) : null;
                    const displayValue = current != null && !Number.isNaN(current)
