...
check('child-date reflow does not send manufacturing duration or end', () => {
...
check('welding date edit never sends schedule keys', () => {
    assert.deepEqual(
        deptSchedulePatch('welding', new Set(['start_date', 'end_date']), imalat),
        {},
    );
});
