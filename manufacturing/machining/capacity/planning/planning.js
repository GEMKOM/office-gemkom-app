...
async function selectMachine(machineId, machineName) {
    ...
    if (isLoadingMachine) {
        ...
        showNotification('Makine yükleniyor, lütfen bekleyin...', 'info');
        return;
    }
    ...
    try {
        ...
        statusBadge.innerHTML = '<i class="fas fa-check-circle me-1"></i>Seçili';
...
function renderTasksTable(tasks) {
    ...
    tbody.innerHTML = sortedTasks.map(task => `
        <tr class="task-row" data-task-key="${task.key}">
            <td>
...
function renderGanttChart(tasks) {
    ...
    const dates = sortedTasks
        .filter(task => task.planned_start_ms && task.planned_end_ms)
        .map(task => ({
            start: new Date(task.planned_start_ms),
            end: new Date(task.planned_end_ms)
        }));
    ...
    if (dates.length === 0) {
        ganttContainer.innerHTML = `
            <div class="text-center text-muted py-5">
                ...
                <p>Görevler için tarih bilgisi bulunmuyor</p>
            </div>
        `;
        return;
    }
            ...
            <div class="gantt-timeline" style="width: ${totalDays * dayWidth}px;">
                <div class="gantt-header-row">
                    ${Array.from({length: totalDays}, (_, i) => {
                        ...
                        return `
                            <div class="gantt-header-cell" style="width: ${dayWidth}px;">
                                <div class="gantt-date">${date.getDate()}</div>
                                <div class="gantt-month">${date.toLocaleDateString('tr-TR', { month: 'short' })}</div>
                            </div>
                        `;
                    }).join('')}
                </div>
                ...
                <div class="gantt-task-row">
                    ...
                    <div class="gantt-task-bar-container">
                        <div class="gantt-task-bar ${task.plan_locked ? 'locked' : 'unlocked'}" 
                             ...
                             title="${task.name} (${startDate.toLocaleDateString('tr-TR')} - ${endDate.toLocaleDateString('tr-TR')})">
...
function setupGanttDragAndDrop() {
    ...
    timelineCells.forEach(cell => {
        ...
        cell.addEventListener('drop', (e) => {
            ...
            const cellIndex = Array.from(timelineCells).indexOf(cell);
            const newStartDate = new Date(minDate);
            newStartDate.setDate(newStartDate.getDate() + cellIndex);
...
function updateTaskDates(taskKey, newStartDate) {
    ...
    const duration = task.planned_end_ms && task.planned_start_ms 
        ? (task.planned_end_ms - task.planned_start_ms) 
        : (task.estimated_hours || 2) * 60 * 60 * 1000; // Default 2 hours

    task.planned_start_ms = newStartDate.getTime();
    task.planned_end_ms = newStartDate.getTime() + duration;
    
    hasUnsavedChanges = true;
    renderGanttChart(currentTasks.filter(t => t.in_plan));
    renderTasksTable(currentTasks.filter(t => t.in_plan));
}
...
function addToPlan(taskKey) {
    ...
    if (!task.planned_start_ms) {
        const now = new Date();
        task.planned_start_ms = now.getTime();
        task.planned_end_ms = now.getTime() + (task.estimated_hours || 2) * 60 * 60 * 1000;
    }
...
async function confirmAutoschedule() {
    ...
    sortedTasks.forEach((task, index) => {
        const duration = (task.estimated_hours || 2) * 60 * 60 * 1000; // Convert hours to milliseconds
