        ...
        columns: [
            ...
            },
            {
                ...
                formatter: (value, row) => {
                    ...
                    if (issueKey) {
                        const cutUrl = `/manufacturing/cnc-cutting/cuts?cut=${encodeURIComponent(issueKey)}`;
                        return `<a href="${cutUrl}" target="_blank" style="font-weight: 700; color: #0d6efd; font-family: 'Courier New', monospace; font-size: 1rem; background: rgba(13, 110, 253, 0.1); padding: 0.25rem 0.5rem; border-radius: 4px; border: 1px solid rgba(13, 110, 253, 0.2); text-decoration: none; cursor: pointer; display: inline-block; transition: all 0.2s ease;" onmouseover="this.style.background='rgba(13, 110, 253, 0.2)'; this.style.color='#0a58ca';" onmouseout="this.style.background='rgba(13, 110, 253, 0.1)'; this.style.color='#0d6efd';" title="Kesim detaylarını görüntüle">${value}</a>`;
                    }
...
async function loadMachines() {
    try {
        const machinesResponse = await fetchMachines(1, 100, { used_in: 'cnc_cutting' });
...
async function loadUsers() {
    try {
        users = await fetchUsers('cnc_cutting_team');
...
async function loadTimers(page = 1) {
    try {
        ...
        const response = await authedFetch(`${backendBase}/cnc_cutting/timers/?${query}`);
...
function buildTimerQuery(page = 1) {
    ...
    const nestingIdFilter = filterValues['nesting-id-filter'];
    if (nestingIdFilter) {
        params.append('nesting_id', nestingIdFilter);
    }
...
async function deleteTimer(timerId) {
    try {
        const response = await authedFetch(`${backendBase}/cnc_cutting/timers/${timerId}/`, {
            method: 'DELETE'
        });
...
function showEditTimerModal(timer) {
    ...
    const formData = {
        ...
        thickness_mm: timer.thickness_mm ? `${timer.thickness_mm} mm` : '',
...
function showTaskInfoModal(timerData, taskDetails = null) {
    ...
    const modalContent = `
        ...
        <div class="row">
            <div class="col-md-6">
                <strong>Nesting ID:</strong><br>
                <span>${displayData.nesting_id || 'Belirtilmemiş'}</span>
            </div>
            <div class="col-md-6">
                <strong>Kalınlık:</strong><br>
                <span>${displayData.thickness_mm ? displayData.thickness_mm + ' mm' : 'Belirtilmemiş'}</span>
            </div>
        </div>
        <hr>
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <span>${displayData.issue_key || 'Belirtilmemiş'}</span>
            </div>
        </div>
        `}
    ...
    const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('task-info-modal'));
    modal.show();
}
...
function exportTimers() {
    try {
        ...
        const exportUrl = `${backendBase}/cnc_cutting/timers/export/?${query}`;
