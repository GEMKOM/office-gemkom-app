...
async function showUserDetails(user) {
    ...
    const hasPartsCompleted = user.total_parts_completed !== undefined;
    const hasPartsWorked = user.total_parts_worked_on !== undefined;
    const hasHoldHours = user.total_hold_hours !== undefined && user.total_hold_hours > 0;

    const statCount = 4 + (hasTasksCompleted ? 1 : 0) + (hasTasksWorked ? 1 : 0) +
        (hasPartsCompleted ? 1 : 0) + (hasPartsWorked ? 1 : 0) + (hasHoldHours ? 1 : 0);
    ...
    if (hasPartsCompleted) {
        statsContent += `
            <div class="col-md-${colSize}">
                <div class="stat-item" style="padding: 0.5rem;">
                    <div class="stat-label" style="font-size: 0.75rem; margin-bottom: 0.25rem;">
                        <i class="fas fa-cubes me-1"></i>Tamamlanan Parça
                    </div>
                    <div class="stat-value text-primary" style="font-size: 1.25rem;">${user.total_parts_completed}</div>
                </div>
            </div>
        `;
    }
    ...
    if (hasPartsWorked) {
        statsContent += `
            <div class="col-md-${colSize}">
                <div class="stat-item" style="padding: 0.5rem;">
                    <div class="stat-label" style="font-size: 0.75rem; margin-bottom: 0.25rem;">
                        <i class="fas fa-cube me-1"></i>İşlem Parçası
                    </div>
                    <div class="stat-value text-primary" style="font-size: 1.25rem;">${user.total_parts_worked_on}</div>
                </div>
            </div>
        `;
    }
    ...
    return [
        {
            ...
            formatter: (value) => {
                if (!value) return '-';
                return `<a href="/manufacturing/cnc-cutting/reports/finished-timers/?edit=${value}" target="_blank" rel="noopener noreferrer" class="badge bg-secondary text-decoration-none" style="cursor: pointer;">#${value}</a>`;
            }
        },
        {
            field: 'start_time',
        ...
        },
        {
            ...
            formatter: (value) => {
                ...
                return `<a href="/manufacturing/cnc-cutting/cuts/?filter=${q}" target="_blank" rel="noopener noreferrer" class="badge bg-primary text-decoration-none" style="cursor: pointer;">${value}</a>`;
            }
        },
        {
            field: 'task_name',
